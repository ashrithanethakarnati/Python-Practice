...
# ---------------------------
# Helper: Try to open Chrome to the Streamlit URL (cross-platform)
# ---------------------------
def try_open_chrome(url="http://localhost:8501"):
    def _open():
        time.sleep(1.0)
        ...
        try:
            if system == "windows":
                ...
                for cmd in (["google-chrome", url], ["google-chrome-stable", url],
                            ["chromium-browser", url], ["chromium", url], ["chrome", url]):
                    try:
...
# ---------------------------
def atm():
    st.set_page_config(page_title="ATM Simulator", layout="centered", initial_sidebar_state="auto")
    ...
    # Sidebar: show basic info & reset/test controls
    with st.sidebar:
        st.header("ATM Controls")
        st.write(f"Balance: ₹{st.session_state.balance:.2f}")
        st.write(f"PIN Attempts used: {st.session_state.attempts} / 3")
        if st.button("Reset Session (clear state)"):
            for key in list(st.session_state.keys()):
                del st.session_state[key]
            st.experimental_rerun()
        st.markdown("---")
        st.caption("This demo uses a default PIN `1234`. You can change it in the code.")
